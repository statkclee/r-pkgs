...
The code won't work because ggplot2's `qplot()` function won't be available: `library(foo)` doesn't re-execute `library(gpplot2)`. The top-level R code in a package is only executed when the package is built, not when it's loaded.
